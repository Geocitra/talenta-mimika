#!/usr/bin/env python3
# bum.py — Best Practice Codebase Aggregator (NestJS / Next.js / TypeScript / Prisma Monorepo)
import os
import sys
import re
import argparse
...
# Menyimpan seluruh konfigurasi penapisan secara terpusat untuk Fullstack Monorepo
# =========================================================================
class AggregatorConfig:
    ...
    FORBIDDEN_DIRS = {
        ...
        "uploads", ".agents", ".claude", ".cursor", ".devin", ".gemini"
    }
    ...
    # 3. Ekstensi Berkas Teks yang Diizinkan (Fullstack TS / NestJS / Next.js / Prisma / Scripts)
    INCLUDE_EXTENSIONS = {
        ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".json", ".prisma",
        ".yaml", ".yml", ".md", ".sh", ".sql", ".css", ".scss", ".ps1",
        ".py", ".html", ".mmd"
    }

    # 4. Berkas Konfigurasi & Dokumentasi Penting
    ESSENTIAL_ROOT_FILES = {
        ...
        "docker-compose.yml", "Dockerfile", ".env.example", "schema.prisma",
        "index.js", "server.js", "app.js", "nodemon.json", "jest.config.js",
        ".prettierrc", "README.md", "PROJECT_STATE.md", "AGENTS.md", "CLAUDE.md"
    }

    # 5. Batas Maksimum Ukuran Berkas (1.5 MB)
    MAX_FILE_SIZE_BYTES = 1536 * 1024  

    # Pre-compiled Regex untuk Sensor Keamanan (Mencegah Kebocoran Kredensial Nyata)
    SENSITIVE_REGEX = re.compile(
        r"(\.env($|\.local|\.production|\.development|\.test)|key.*\.pem|.*\.key|id_rsa.*|credentials.*)", 
        re.IGNORECASE
    )
...
class LLMContextOptimizer:
    ...
    def strip_js_ts_comments(text: str) -> str:
        """Menghapus komentar // dan /* ... */ secara aman tanpa merusak string literal atau Dekorator NestJS."""
        pattern = re.compile(
            r'/\*.*?\*/|//.*?$|\'(?:\\.|[^\\\'])*\'|"(?:\\.|[^\\"])*"|`(?:\\.|[^\\`])*`',
...
class CodebaseAggregator:
    ...
    def should_ignore(self, path: Path, is_dir: bool = False) -> bool:
        ...
        if not is_dir:
            ...
            # Izinkan template environment (.env.example, .env.sample)
            if path.name.endswith(".example") or path.name.endswith(".sample"):
                pass
            elif self.config.SENSITIVE_REGEX.match(path.name):
                return True
                
            # Filter Berkas Pengujian (Unit Test/E2E/PS1) jika parameter diaktifkan
            if self.exclude_tests:
                lower_name = path.name.lower()
                if "test" in lower_name or "spec" in lower_name:
                    if path.suffix in (".ts", ".tsx", ".js", ".jsx", ".ps1"):
                        return True

        # 3. Filter folder khusus pengujian secara mutlak jika parameter diaktifkan
    ...
    def execute(self):
        ...
        if self.exclude_tests:
            print("🚫 Berkas pengujian (*.test.*, *.spec.*, test_*.ps1) diabaikan.")
        ...
        try:
            with open(self.output_file, "w", encoding="utf-8") as out_file:
                out_file.write("=== MIMIKA TALENTA CODEBASE BUNDLE (FULLSTACK MONOREPO) ===\n\n")

                for root, dirs, files in os.walk(self.target_path):
                    root_path = Path(root)

                    # 1. Pangkas folder terlarang (node_modules, .git, dist, .next, uploads, dll)
                    ...
                    for file_name in files:
                        ...
                        # Penapisan B: Saring Ekstensi yang diizinkan / Berkas Konfigurasi Penting
                        is_allowed_ext = file_path.suffix in self.config.INCLUDE_EXTENSIONS
                        is_essential_file = file_name in self.config.ESSENTIAL_ROOT_FILES
                        if not is_allowed_ext and not is_essential_file:
                            continue

                        # Penapisan C: Cheap Binary Guard
                        if self.optimizer.is_binary(file_path):
                            continue

                        # Penapisan D: Size Guard
                        try:
            ...
            if processed_files_stats:
                print(f"\n📦 Ringkasan: {file_count} berkas berhasil diambil dari seluruh monorepo (apps/backend, apps/frontend, packages, tests).")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Optimasi Bundel Kode Fullstack Mimika Talenta (NestJS / Next.js / TypeScript / Prisma Monorepo)")
    ...
    parser.add_argument("--exclude-tests", action="store_true", help="Jangan sertakan berkas pengujian (*.test.*, *.spec.*, test_*.ps1)")
